...
pub fn get_timetable(group_map: serde_json::Value) -> serde_json::Value {
    // getting group data from group_map by key group

    // if !&group_coords.is_null() { по идее не понадобиться, так как в group_map есть только те группы, которые есть в расписании
    //     println!("Group found: {}", group);
    // } else {
    //     println!("Group not found: {}", group);
    // }

    let mut groups_timetable= json!({});
    ...
    for (key, group_coords) in group_map.as_object().unwrap() {
        ...
        groups_timetable[key] = json_timetable;
    }
    groups_timetable
}